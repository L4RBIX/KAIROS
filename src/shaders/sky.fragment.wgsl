...
fn main(input: FragmentInputs) -> FragmentOutputs {
    ...
    //
    // Under heavy overcast the disc must not sit as a hard white hole. Fade only
    // the runtime disc/aureole — never S.sunIntensity — so the baked sky LUT
    // stays coherent with direct lighting.
    let mu = dot(dir, uniforms.sunDir);
    let sunVis = 1.0 - uniforms.cloudAmount * 0.94;
    let discCos = cos(0.0046);
    if (mu > discCos && sunVis > 0.001) {
        let r = sqrt(max(0.0, 1.0 - mu * mu)) / 0.0046;
        let limb = pow(max(0.0, 1.0 - r * r * 0.72), 0.42);
        col += uniforms.sunColor * uniforms.sunIntensity * 42.0 * limb * sunVis;
    }
    let aureole = pow(max(0.0, mu), 1400.0) * 5.5 + pow(max(0.0, mu), 64.0) * 0.28;
    col += uniforms.sunColor * uniforms.sunIntensity * aureole * 0.5 * sunVis;
